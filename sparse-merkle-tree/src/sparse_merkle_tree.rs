#![deny(warnings)]
use crate::common::{hash_one_thing, hash_two_things, zero_digest, Digest};
...
pub enum SparseMerkleTreeProof {
    NotPresent { next_leaf: SparseMerkleTreeNodeLeaf },
    Present { leaf: SparseMerkleTreeNodeLeaf },
}
...
impl AuthenticatedKV for SparseMerkleTree {
                ...
                for _ in 0..height + 1 {
                    if (path >> height) & 1 == 0 {
                        node = SparseMerkleTreeNodeBranch {
                            ...
                            right: None,
                            hash: smk_util::hash_branch(
                                &node.clone().hash,
                                &smk_util::empty_smt_hash(),
                            ),
                            leaf: None,
                        };
                    } else {
    ...
    fn get(&self, key: Self::K) -> (Option<Self::V>, Self::LookupProof) {
        ...
        if node.hash == smk_util::empty_smt_hash() {
            return (
                None,
                SparseMerkleTreeProof::NotPresent {
                    next_leaf: next_leaf,
                },
            );
        }
        ...
        if node.leaf.is_some() {
            ...
            if leaf.hash == smk_util::hash_leaf(&key) && leaf.key == key {
                return (
                    Some(leaf.value.clone().unwrap()),
                    SparseMerkleTreeProof::Present { leaf: leaf },
                );
            }
        }
...
mod smk_util {
    ...
    pub fn hash_branch<T: std::convert::AsRef<[u8]>>(
        left: &T,
        right: &T,
    ) -> Digest {
